...
exports.isNutricionista = (req, res, next) => {
  if (!['admin', 'nutricionista'].includes(req.user.role)) {
    return res.status(403).json({ msg: 'Acesso apenas nutricionista' });
  }
  next();
};

exports.isPersonal = (req, res, next) => {
  if (!['admin', 'personal'].includes(req.user.role)) {
    return res.status(403).json({ msg: 'Acesso apenas personal' });
  }
  next();
};
